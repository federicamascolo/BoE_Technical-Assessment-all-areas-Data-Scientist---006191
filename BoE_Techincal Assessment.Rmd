...
Firm size: GWP and Total assets have been used as proxies for firm size. The histograms below show the first 40 firms with the highest values of GWP and Total assets for 2020. The scatterplots show the two measures jointly. On the x-axes the GWP in 2020. 
...
# SCR coverage ratio as measure of risk: 
Firms with SCR coverage ratio lower than one should not meet the capital requirements. Therefore is important to check which firms are in this position. 96% of the firms report SCR coverage ratio equal to zero and in the histogram are reported only firms with negative or lower than one value. 
...
The change between 2020 and 2019 in Gross claims incurred can be considered if the objective is to allocate resources to important firms. The variable indicates a large cost for the insurer to check over time.
```{r , echo = FALSE}
setwd("C:/Users/feder/Desktop/Applications/Applications varie/21-22/BoE_technical assessment")
df_under <- read.xlsx("Data for technical assessment.xlsx", sheet = "Dataset 2 - Underwriting")
```

The first histograms show the first two firms with the biggest percentage change in the gross claims incurred between 2019 and 2020. While the second shows from the third to the 40th firm with the bigger percentage change in the same variable. Finally, the scatterplot shows firms with higher values for total assets and Gross claims incurred. 
```{r , echo = FALSE}
...
Considering total assets and Gross claims incurred as the most important measure to allocate resources to a firm, the dataset below shows the biggest firms in terms of total assets and those with the highest claims. To validate the analysis the dataset shows the percentage deviation from the mean values and the variance by firm of both variables 
```{r, echo = FALSE }
